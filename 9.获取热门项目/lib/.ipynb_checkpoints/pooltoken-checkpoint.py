...
if __name__ == "__main__":
    ...
    result = []
    thread_list = []
    with open('./doc/地址.txt','r',encoding='utf-8') as f:
        addrs = f.read()
        for addr in tqdm(addrs.split('\n')):
            #爬取池子余额
            pool_sema.acquire()
            thread = threading.Thread(target=GetPool(addr).request_)
            thread.start()
    ...
    df.sort_values('数量',ascending=False).reset_index(drop=True).to_csv('线索余额.csv')